...
///
Future<AlertDialog> victoryDialog(BuildContext context, String winnerPlayer, String homePlayerID,
    Function(bool isFirstPlayerToLeave, BuildContext context) onExit, VoidCallback onNewMatch) async {
  var response = await http.get("https://api.forismatic.com/api/1.0/?method=getQuote&format=json&lang=en");
  final parsed = json.decode(response.body.replaceAll(r"\'", "'").replaceAll(r"\~'", "~"));
      ...
      builder: (_) {
        return AlertDialog(
            title: Column(children: <Widget>[
              (winnerPlayer == homePlayerID) ? Text('You are the winner!') : Text('You lost the game!'),
            ]),
...
final _quoteStyle = TextStyle(color: Colors.black, fontWeight: FontWeight.w100, fontStyle: FontStyle.italic);
